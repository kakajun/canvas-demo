<template>
  <div class="edit-anchor-zone">
    <div class="edit-toolbar">
      ...
      <span
        class="toolbar-icon-wrap"
        :class="{ icon_active: isTableActive }"
        v-if="!setAnchorFlag"
      >
        <el-tooltip
          class="item"
        ...
        >
          <el-button
            ><i
              class="iconfont icon-OCR-table toolbar-icon"
              @click="drawTable"
            ></i
          ></el-button>
        </el-tooltip>
      ...
      <span class="toolbar-icon-wrap" :class="{ icon_active: isdrawActive }">
        <el-tooltip
          ...
          placement="top"
        >
          <el-button @click="dragDraw">拖拽画布</el-button>
        </el-tooltip>
      </span>
      ...
      <span
        class="toolbar-icon-wrap"
        :class="{ icon_active: isAdaptDrawActive }"
      >
        <el-tooltip
          class="item"
          effect="light"
          ...
          placement="top"
        >
          <el-button @click="adaptDraw">适应</el-button>
        </el-tooltip>
      </span>
      <span
        class="toolbar-icon-wrap"
        :class="{ icon_active: isSetOriginActive }"
      >
        <el-tooltip class="item" effect="light" content="原图" placement="top">
          <el-button @click="setOriginSize">原图</el-button>
        </el-tooltip>
      </span>
    </div>
    <div class="edit-body">
...
export default {
  ...
  methods: {
    ...
    computeInitData() {
      ...
      this.initScaleVal = this.currentScaleVal =
        widthRatio >= heightRatio ? heightRatio : widthRatio
      if (this.initScaleVal < 0.1) {
        this.initScaleVal = 0.1
        this.currentScaleVal = 0.1
      }
    ...
    zoomIn() {
      ...
      this.adapt(
        this.currentScaleVal,
        parseFloat(this.currentScaleVal) + this.step
      )
    ...
    _drawRect() {
      ...
      this.baseTarget.onmousedown = e => {
        ...
        this.selectId = (
          this.setAnchorFlag ? this.rectList1 : this.rectList2
        ).length
        moveIn = true
    ...
    drawRectWithColor(instance, x, y, width, height, id) {
      ...
      this.initDrawRect()
      ;(this.setAnchorFlag ? this.rectList1 : this.rectList2)[id] = {
        x,
        y,
        width,
        height,
      ...
      ;(this.setAnchorFlag ? this.rectList1 : this.rectList2).map(item => {
        if (item.id === id) {
          ...
          pointList.map(item => {
            ...
            instance.fill()
            instance.strokeStyle = this.setAnchorFlag
              ? this.color1
              : this.color2
            instance.lineWidth = this.lineWidth
            instance.globalAlpha = 1
    ...
    reShowRect(moveX, moveY, moved, id, cursorStr) {
      ...
      switch (cursorStr) {
        ...
        case 'revise':
          ...
          ;(this.setAnchorFlag ? this.rectList1 : this.rectList2).map(
            (item, index, arr) => {
              if (item.id === id) {
                if (
    ...
    _drawTable() {
      ...
      this.baseTarget.onmousemove = e => {
        ...
        // 新建拖拉选框
        if (moveIn && !moved) {
          return this.drawTableWithColor(
            this.baseInstance,
            mouseInit.x,
            mouseInit.y,
            move.x,
      ...
      this.tableList.map(item => {
        if (item.id === id) {
          ...
          const centerLine = item.centerLineList.some((rateX, i) => {
            index = i
            const xFlag =
              x > item.x + item.width * rateX - this.circlsRadius &&
              x < item.x + item.width * rateX + this.circlsRadius
            const yFlag =
              y > item.y + item.height / 2 - this.circlsRadius &&
              y < item.y + item.height / 2 + this.circlsRadius
            return xFlag && yFlag
          })
          ...
          // 使用当前 cursor 状态
          if (this.currentCursor) {
            return this.throttle(this.reShowTable)(
              moveX,
              moveY,
              moved,
              id,
          ...
          if (ltCircle) {
            ...
            this.throttle(this.reShowTable)(
              moveX,
              moveY,
              moved,
              id,
        ...
        case 'centerLine':
          ...
          moved &&
            this.tableList.map(item => {
              ...
              if (
                item.x + item.width * rateX + moveX <=
                item.x + this.circlsRadius
              ) {
                return (item.centerLineList[this.centerLineIndex] =
                  (this.circlsRadius + 1) / item.width)
              }
              if (
                item.x + item.width * rateX + moveX >=
                item.x + item.width - this.circlsRadius
              ) {
                return (item.centerLineList[this.centerLineIndex] =
                  (item.width - this.circlsRadius - 1) / item.width)
              }
